...
export async function renderFriends(options = {}) {
  ...
  if (!options.skipLoading) {
    view.innerHTML = `<section class="page"><div class="friends-shell friends-dashboard glass"><section class="friends-hero"><div class="friends-hero-copy"><div class="kicker">STUDY CIRCLE</div><h1>Friends</h1><p class="sub">Build your study circle, manage requests, and compare reviewer progress.</p></div><div class="friends-stat-strip"><div class="friends-stat-tile"><b>...</b><span>Friends</span></div><div class="friends-stat-tile"><b>...</b><span>Incoming</span></div><div class="friends-stat-tile"><b>...</b><span>Sent</span></div></div></section><div class="friends-dashboard-grid"><div class="friends-stack"><section class="friends-panel"><div class="friends-panel-head"><h3>Incoming Requests</h3><span>...</span></div><div class="friends-rows"><div class="friends-empty-note">Loading incoming requests.</div></div></section><section class="friends-panel"><div class="friends-panel-head"><h3>Sent Requests</h3><span>...</span></div><div class="friends-rows"><div class="friends-empty-note">Loading sent requests.</div></div></section></div><section class="friends-panel find-classmates-panel"><div class="friends-panel-head"><h3>Find Classmates</h3></div><div class="friend-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6.5"></circle><path d="m16 16 4 4"></path></svg><span><input id="friendSearchInput" type="search" aria-label="Search classmates" placeholder="Search by username or full name" value="${escapeHTML(state.friendsSearchQuery)}"></span><kbd>/</kbd></div><p class="friends-search-helper">Syncing classmates.</p><div class="friends-rows"><div class="friends-empty-note friend-empty-visual"><div><b>Loading study circle</b><span>Checking Supabase friendships.</span></div></div></div></section><section class="friends-panel friends-list-panel"><div class="friends-panel-head"><h3>My Friends</h3><span>...</span></div><div class="friends-rows"><div class="friends-empty-note">Loading friends.</div></div></section></div></div></section>`;
    animateSurface();
  }
  ...
  view.innerHTML = `<section class="page"><div class="friends-shell friends-dashboard glass"><section class="friends-hero"><div class="friends-hero-copy"><div class="kicker">STUDY CIRCLE</div><h1>Friends</h1><p class="sub">Build your study circle, manage requests, and compare reviewer progress.</p>${errorMessage ? `<p class="friends-error">${escapeHTML(errorMessage)}</p>` : ''}</div><div class="friends-stat-strip"><div class="friends-stat-tile"><b>${friendItems.length}</b><span>${friendItems.length === 1 ? 'Friend' : 'Friends'}</span></div><div class="friends-stat-tile"><b>${friendState.incoming.length}</b><span>Incoming</span></div><div class="friends-stat-tile"><b>${friendState.sent.length}</b><span>Sent</span></div></div></section><div class="friends-dashboard-grid"><div class="friends-stack"><section class="friends-panel"><div class="friends-panel-head"><h3>Incoming Requests</h3><span>${friendState.incoming.length}</span></div><div class="friends-rows">${incomingHTML}</div><button class="friends-panel-link" type="button">View all incoming &rarr;</button></section><section class="friends-panel"><div class="friends-panel-head"><h3>Sent Requests</h3><span>${friendState.sent.length}</span></div><div class="friends-rows">${sentHTML}</div><button class="friends-panel-link" type="button">View all sent &rarr;</button></section></div><section class="friends-panel find-classmates-panel"><div class="friends-panel-head"><h3>Find Classmates</h3></div><div class="friend-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6.5"></circle><path d="m16 16 4 4"></path></svg><span><input id="friendSearchInput" type="search" aria-label="Search classmates" placeholder="Search by username or full name" value="${escapeHTML(state.friendsSearchQuery)}"></span><kbd>/</kbd></div><p class="friends-search-helper">Type at least 2 characters to search.</p><div class="friends-rows">${searchHTML}</div></section><section class="friends-panel friends-list-panel"><div class="friends-panel-head"><h3>My Friends</h3><span>${friendItems.length}</span></div><div class="friends-rows">${friendsHTML}</div><button class="friends-panel-link" type="button">View all friends &rarr;</button></section></div></div>${detailHTML}</section>`;
  setupFriendSearch();
  animateSurface();
}
